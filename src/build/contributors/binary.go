...
func (b *binaryContributor) signChecksums(rc *domains.RunContext, t config.TargetConfig, checksumPath string) (string, error) {
	...
	sc, serr := autosign.ResolveSigningContext(rc.Ctx, cfg, profile, rc.RootDir, rc.RootDir, rc.Config.Toolchains.Want, now)
	if serr != nil {
	...
	err := cosign.SignBlob(rc.Ctx, rc.RootDir, rc.Config.Toolchains.Want, checksumPath, sigPath, sc.Plan, sc.Env)
	if err != nil {
		rc.RB.Record(artifactID, artifact.Outcome{
