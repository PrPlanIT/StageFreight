...
func TestGateErrorSince(t *testing.T) {
	...
	if GateErrorSince(findings, map[string]bool{findings[1].Fingerprint(): true}, "main") == nil {
	...
	if err := GateErrorSince(findings, map[string]bool{findings[2].Fingerprint(): true}, "main"); err != nil {
