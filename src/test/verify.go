...
// plus the full TestResult so the caller can record structured facts (counts,
// coverage, failing test names) into cistate.
//
...
func Verify(ctx context.Context, cfg *config.Config, rootDir string, w io.Writer, intent Intent) (bool, *TestResult, error) {
	...
	res := RunRender(ctx, suites, rootDir, cfg.Toolchains.Want, w, intent)
	return !res.FailedNonAdvisory(), res, nil
}
