...
//
// Note: an SSH agent (SSH_AUTH_SOCK) or an on-disk key is NOT an explicit
// StageFreight credential — it is part of the user's Git environment, which system
// git uses directly. Only an injected in-memory key counts.
func ResolveTransport(remoteURL string) (TransportDecision, error) {
	// System git is the transport authority for repository-local workflows. Both
	// conditions for it live here so the decision is one model in one place — a git
	// binary must be available to delegate to, AND StageFreight must not have been
	// handed a credential to act as. Selection never re-derives either half.
