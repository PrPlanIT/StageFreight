...
> 1. **Preset resolution on the run/build path** — ✅ **FIXED** (`44bdc3b`): built `loadResolved` as the single construction path — it now resolves `preset:`/`presets:` before decode (reusing `ResolvePresets` + the reporter's `localPresetLoader`, so runtime resolves IDENTICALLY to the report — split-brain closed). `LoadWithReport` reuses `loadResolved`'s provenance entries; its resolve-and-discard is removed (one resolve site). Added a `preset_source` field (since **removed** — provenance lives on each reference); preset-free configs decode original bytes (no round-trip). Tests: resolve-on-run-path · local-override · preset_source · preset-free-unchanged. (SUPERSEDED — runtime source resolution is now built: `66ce39d`…`d76e949`. A satellite resolves each reference itself; governance materializes ahead of time as an optimization, not a dependency, so a failed governance run delays policy instead of freezing it.)
...
> - **Call-back (resolution) — PER REFERENCE, any source.** A reference carries its own `Source`/`Ref`/`Path`, so one config may name several sources at once. Forms: `preset/x.yml` (local) · `https://host/x.yml` (the URL **is** the source; no revision semantics, so tracked) · `gitlab:Org/Repo//preset/x.yml[@ref]` (git). No ref = the source's **default branch (tracked)**; `@branch` tracks that branch's HEAD; `@refs/tags/…` or a sha **pins**. Pinning is the operator's opt-out from tracking, never the default. Cycle + path-traversal guards. (`src/presetref/presetref.go` · `src/presetfetch/` · `src/config/report.go:31-51`)
> - **Cache — evidence, never authority.** On-disk git-committed `.stagefreight/preset-cache/`, keyed by the reference's own identity (`source@ref/path`) so two sources sharing a filename cannot collide. Authority belongs to the **reference**: tracked → upstream, pinned → the named revision, local → the working tree; the retained copy is the last successful resolution, used as fallback when the source is unreachable. Resolution is ONE operation for every kind — fetch, reconcile against what was retained, then let the kind pick the policy: **tracked + differs = drift** (refresh, report, republish at publish) · **pinned + differs = violation** (`PinViolation` carrying both copies — the retained copy IS the checksum) · **unreachable = fall back**. `preset_source:`/`cache_policy:` are **deleted**: a stamped policy could only contradict the reference. (`src/presetref/resolver.go` · `src/governance/distributor.go` · `src/cli/cmd/ci_presets.go`)
...
> - ✅ **FIXED (`44bdc3b`) — the run path now resolves presets.** Built the `loadResolved` chokepoint the invariant always promised: `LoadWithWarnings` → `loadResolved` does Unmarshal → `ResolvePresets` (same `localPresetLoader` the reporter uses) → decode the resolved map. `LoadWithReport` now consumes `loadResolved`'s provenance entries — the resolve-and-discard is gone, one resolve site. Added a `preset_source` field (since **removed** — see the Cache bullet); preset-free configs decode original bytes verbatim. Runtime resolves IDENTICALLY to `config resolve` now — split-brain closed. (Still open, separately: restore `invariants.md §1`'s enforcement test `invariants_test.go`, which asserts no raw Config decode outside the chokepoint — the behavioral tests guard the fix; the structural ratchet is a nice-to-have.)
